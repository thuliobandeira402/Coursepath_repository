...
def check_email_existe(email):
    """Função para verificar se o email já existe no banco de dados, utilizada para evitar cadastro de emails iguais"""
    connection = sqlite3.connect('banco.db')
    cursor = connection.cursor()
    cursor.execute("SELECT email FROM contas_curso WHERE email = ?", (email,))
